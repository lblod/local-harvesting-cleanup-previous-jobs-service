...
const MAX_FILE_DELETE_BATCH_ITERATIONS = 10000;

async function countFileOnDiskForContainer(jobUri, containerPred) {
  const res = await query(`
    SELECT (COUNT(DISTINCT ?fileOnDisk) AS ?count) WHERE {
      GRAPH <${DEFAULT_GRAPH}> {
        ?task <http://purl.org/dc/terms/isPartOf> ${sparqlEscapeUri(jobUri)} .
  ...
  `, {}, connectionOptions);
  return parseInt(res.results.bindings[0].count.value, 10);
}

async function countFileForContainer(jobUri, containerPred) {
  const res = await query(`
    SELECT (COUNT(DISTINCT ?file) AS ?count) WHERE {
      GRAPH <${DEFAULT_GRAPH}> {
        ?task <http://purl.org/dc/terms/isPartOf> ${sparqlEscapeUri(jobUri)} .
        ?task <${containerPred}> ?container .
        ?container <http://redpencil.data.gift/vocabularies/tasks/hasFile> ?file .
...
export async function deleteContainersForJob(jobUri) {
  ...
  for (const containerPred of containerPreds) {
    // Batch the physical file delete to stay under Virtuoso's vectored
    // execution cap (FRVEC ... > 1000000). Deleting ?fileOnDisk ?p ?o also
    // removes the qualifying nie:dataSource triple, so processed resources
    // drop out of the SELECT on the next pass.
    let iteration = 0;
    let remaining = await countFileOnDiskForContainer(jobUri, containerPred);
    while (remaining > 0) {
      if (iteration >= MAX_FILE_DELETE_BATCH_ITERATIONS) {
        throw new Error(
          `deleteContainersForJob: exceeded MAX_FILE_DELETE_BATCH_ITERATIONS (${MAX_FILE_DELETE_BATCH_ITERATIONS}) for ${containerPred} on ${jobUri}; ${remaining} file-on-disk resources still remaining`,
        );
      }
      console.log(
        `deleteContainersForJob: ${containerPred} batch ${iteration + 1}, ${remaining} file-on-disk resources remaining for ${jobUri}`,
      );
      await update(`
        DELETE { GRAPH <${DEFAULT_GRAPH}> { ?fileOnDisk ?p ?o } }
        WHERE {
          {
            SELECT DISTINCT ?fileOnDisk WHERE {
              GRAPH <${DEFAULT_GRAPH}> {
                ?task <http://purl.org/dc/terms/isPartOf> ${sparqlEscapeUri(jobUri)} .
                ?task <${containerPred}> ?container .
                ?container <http://redpencil.data.gift/vocabularies/tasks/hasFile> ?file .
            ...
            } LIMIT ${FILE_DELETE_BATCH_SIZE}
          }
          GRAPH <${DEFAULT_GRAPH}> { ?fileOnDisk ?p ?o . }
        }
      `, {}, connectionOptions);
      iteration++;
      remaining = await countFileOnDiskForContainer(jobUri, containerPred);
    }
    // Batch the virtual file delete. The anchor triple (?container hasFile
    // ?file) has ?file as object, so each batch also deletes that link to make
    // ?file drop out of the next pass. A UNION (rather than OPTIONAL) matches
    // the two cases independently: files with own triples, and the hasFile link
    // for every batched file (so files without own triples still drop out).
    iteration = 0;
    remaining = await countFileForContainer(jobUri, containerPred);
    while (remaining > 0) {
      if (iteration >= MAX_FILE_DELETE_BATCH_ITERATIONS) {
        throw new Error(
          `deleteContainersForJob: exceeded MAX_FILE_DELETE_BATCH_ITERATIONS (${MAX_FILE_DELETE_BATCH_ITERATIONS}) for ${containerPred} on ${jobUri}; ${remaining} virtual file resources still remaining`,
        );
      }
      console.log(
        `deleteContainersForJob: ${containerPred} batch ${iteration + 1}, ${remaining} virtual file resources remaining for ${jobUri}`,
      );
      await update(`
        DELETE { GRAPH <${DEFAULT_GRAPH}> { ?file ?p ?o . ?container <http://redpencil.data.gift/vocabularies/tasks/hasFile> ?file } }
        WHERE {
          {
